...
//		return "pages/login";
	 ...
	 * addtoCart(@RequestParam("categoryid") int categoryid, HttpSession session) {
	 * String referer = request.getHeader("referer"); EcoUser user = (EcoUser)
	 * session.getAttribute("user"); if (session.getAttribute("user") == null) {
	 * return "pages/login"; } Cart cart =
	 * cartService.findCartByUserId(user.getId()); System.out.println(cart.getId());
	 * Orders order = new Orders(); order.setCarts(cart);
	 * order.setCategories(categoryService.findByID(categoryid)); List<Orders>
	 * orders = cart.getOrders().stream().distinct().collect(Collectors.toList());
	 * for (Orders o : orders) if (o.getCategories().getId() == categoryid) {
	 * o.setQuantity(o.getQuantity() + 1); orderService.updateOrder(o); if (referer
	 * != null) { return "redirect:" + referer; } } order.setQuantity(1);
	 * orderService.saveOrder(order); return "redirect:" + referer; }
	 */
	@RequestMapping("all")

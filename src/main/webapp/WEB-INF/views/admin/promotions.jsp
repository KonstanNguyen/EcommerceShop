...
<body>
						...
						<div class="card">
							<div class="card-body">
								...
								<button class="btn btn-success mb-3"
									onclick="openAddDiscountedProductPopup()">Add New
									Product Discount</button>
								<button class="btn btn-success mb-3"
									onclick="openAddDiscountedPopup()">Add New Discount</button>
									...
									<table class="table table-striped">
										...
										<tbody>
											<c:forEach var="p" items="${promotions}">
												<c:set var="categoryCount"
													value="${fn:length(p.categories)}" />
														...
														<td><c:choose>
																<c:when test="${categoryCount < 2}">
																	<button class="btn btn-primary"
																		onclick="editDiscountedProduct(${p.id}, ${p.dealPercent}, '${p.startTime}', '${p.endTime}', '${p.status}')">Edit</button>
																	<button class="btn btn-danger"
																		onclick="deleteDiscountedProduct(${p.id})">Delete</button>
																</c:when>
																<c:otherwise>
	...
	<div class="modal fade" id="addDiscountedModal" tabindex="-1"
		role="dialog" aria-labelledby="addDiscountedProductModalLabel"
		aria-hidden="true">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="addDiscountedModalLabel">Add New
						Discount</h5>
					<button type="button" class="close" data-dismiss="modal"
						aria-label="Close">
						<span aria-hidden="true">&times;</span>
				...
				<div class="modal-body">
					<form id="addDiscountedForm"
						action="${pageContext.request.contextPath}/admin/promotions/add.htm"
						method="post">
						<div class="form-group">
							<label for="percent">Percent</label> <input type="number"
								class="form-control" id="percent" min="0" max="100"
	...
	<div class="modal fade" id="editDiscountedProductModal" tabindex="-1"
		role="dialog" aria-labelledby="editDiscountedProductModalLabel"
		aria-hidden="true">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="editDiscountedProductModalLabel">Edit
						Discount</h5>
					<button type="button" class="close" data-dismiss="modal"
						aria-label="Close">
						<span aria-hidden="true">&times;</span>
				...
				<div class="modal-body">
					<form id="editDiscountedProductForm" action="" method="post">
						<input type="hidden" id="editDiscountedProductId"
							name="discountedProductId">
						<div class="form-group">
							<label for="editProductPercent">Percent</label> <input
								type="number" class="form-control" id="editProductPercent"
								min="0" max="100" name="percent" required>
						</div>
						<div class="form-group">
							<label for="editStartTime">Start Time</label> <input
								type="datetime-local" class="form-control" id="editStartTime"
								name="startTime" required>
						</div>
						<div class="form-group">
							<span class="time-error"></span>
						...
						<div class="form-group">
							<label for="editEndTime">End Time</label> <input
								type="datetime-local" class="form-control" id="editEndTime"
								name="endTime" required>
						</div>
						<div class="form-group">
